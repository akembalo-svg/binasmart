---
url: "https://dashenbanksc.com/dashen-mobile-plus"
title: "Dashen Bank — Dashen Mobile Plus"
source_name: "Dashen Bank"
section: "digital"
lang: "en"
status: "live"
fetchedAt: "2026-09-16"
lastChecked: "2026-09-16"
contentHash: "8a6a09a47e9a9fbb2468e00903eb4bb45894be6a"
generated_by: "ops/packs/fetch-pack.js --pack banking"
packFormat: "2"
---

# Dashen Bank — Dashen Mobile Plus

Dashen Bank — digital — Dashen Mobile Plus. On this page: Dashen Mobile Plus Brand Propositions, Dashen Mobile Plus as an Omni-Channel, WH Questions about Dashen Mobile Plus, Contact Us Dashen Mobile Plus Services | Dashen Bank.

በአማርኛ፦ ካርድ፣ ኤቲኤምና ሞባይል ባንክ — Dashen Mobile Plus። ይህ ገጽ ከዳሽን ባንክ ኦፊሴላዊ ድረ-ገጽ በእንግሊዝኛ የተወሰደ ነው። የወለድ መጠን፣ የአገልግሎት ክፍያ፣ ታሪፍና የምንዛሪ ተመን ያለማስታወቂያ ይለወጣሉ። ይህ ገጽ ከላይ በተጠቀሰው ቀን ተቋሙ ባሳተመው መልኩ ነው። በማንኛውም ቁጥር ላይ ከመወሰንዎ በፊት ባንኩን ያረጋግጡ።

Source: https://dashenbanksc.com/dashen-mobile-plus (official Dashen Bank page, in English), fetched 2026-09-16. Everything below is that page as the institution wrote it — every rate, fee, limit and condition is copied, not restated. Interest rates, fees, tariffs and exchange rates change, often without notice. This is the page exactly as the institution published it on the date above. Confirm with the bank before you act on any figure. BinaSmart is not a bank: this page is information, not advice, and nothing here opens an account, moves money or applies for anything.

# Dashen Mobile Plus

### Dashen Mobile Plus Brand Propositions

Objective

- To create an iconic brand rooted in Ethiopian values

Purpose

- Empower Ethiopians to positively transform our world through the innovative use of technology

Values

- Affiliation

- Cultural Relevance & Symbolism

- Trust & Reliability

- Shared Growth & Progress

### Dashen Mobile Plus as an Omni-Channel

- Dashen Mobile Plus is an Omni-channel seamless banking experience that the customer uses to interact with the bank through various channels such as mobile, internet, USSD, all with the same username and password.

### WH Questions about Dashen Mobile Plus

What is Dashen Mobile Plus?

- Dashen Mobile Plus is an Omni channel payment and commerce platform provided by Dashen Bank to its esteemed customers.

- The channel enables the customer to stay on top of their account 24 x 7 without being limited by time and geographic location.

How can I get registered on Dashen Mobile Plus?

- Customers can visit any of the nearest Dashen Branches/Agents for registration.

- Customers without even visiting branches can easily self register on Dashen Mobile Plus various channels i.e. Mobile App, Internet Banking and USSD

What are the various channels to use Dashen Mobile Plus?

- Mobile Application (available both on App store and play sore)

- Use the following link to download:

For iOS

|
Click Here |

For Android |
Click Here |

- USSD (*996#)

- Internet Banking (Web-site: https://ib.dashenbanksc.com/ )

What can I do with Dashen Mobile Plus?

- Fund Transfers
– Transfer within own account
– Transfer Within Dashen Bank,
– Transfer to Other Bank,
– Transfer to Mobile. (Both Registered and Unregistered).

- Bill payments
– DSTV Subscription
– Ethiopian Air ticket purchase
– Airtime purchase
– School fee payments
– Digital contents (Concert tickets, Great Run tickets and many more)
– Water, electricity (Coming Soon)

- Cash in and cash out at agent locations

- Pay merchant:-
Using Dashen Mobile Plus, you can easily pay merchants, through QR code (I.e. Push payment) and Pull payment.

YouTube Link https://youtu.be/okf8C846uKE

- Bulk disbursement (B2C)
Using Dashen Mobile Plus B2C you can:
– Pay employees salary (Single Debit and Multiple Credit in real time)
– Pay suppliers

- Automate your Supply Chain
– Using Dashen Mobile Plus, you can easily automate your distribution chain and easily collect payments from whole sellers and retailers.

- Download statement
– Using Dashen Mobile Plus, you can easily download statement using, CSV, PDF and RTF file formats.

- Check Balance
–You can check balance of as many accounts you have in the Bank in a single view.

### Contact Us Dashen Mobile Plus Services | Dashen Bank

Dashen Contact Center 24 Hours Support

6333
0911-57-58-27
