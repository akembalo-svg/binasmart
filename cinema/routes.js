...
module.exports = function cinemaRoutes(fastify, { prisma, holds, tickets, checkin, OWNER_KEY, riderBotToken, chapa, BASE_URL, notify }) {
  ...
  fastify.post('/api/cinema/ops/programme', async (req, reply) => {
    ...
    // Source is mandatory: the cinema's own post (with its link) or an official document (name only).
    const sourceUrl = str(b.sourceUrl, 400), sourceName = str(b.sourceName, 120);
    if (!sourceName) return reply.code(400).send({ ok: false, error: 'sourceName required: the cinema\'s own post, its official programme, or "phone call <date>"' });
    if (sourceUrl && !/^https?:\/\//.test(sourceUrl)) return reply.code(400).send({ ok: false, error: 'sourceUrl must be a link' });
