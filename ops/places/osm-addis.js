...
async function overpass(query, tries = 4) {
  for (let i = 0; i < tries; i++) {
    const r = await fetch(API, { method: 'POST', headers: { 'user-agent': UA, 'content-type': 'application/x-www-form-urlencoded' },
...
async function fetchAll() {
  ...
  for (const [id, en] of SUBCITIES) {
    const d = await overpass(`[out:json][timeout:120];area(${3600000000 + id})->.a;(${filters});out ids;`);
    for (const e of d.elements) bySub[e.type + e.id] = en;
    console.log('  ' + en.padEnd(18) + d.elements.length);
  }
  const all = await overpass(`[out:json][timeout:180];area(${CITY_AREA})->.a;(${filters});out tags center;`);
...
function build({ at, elements, bySub }) {
  ...
  const docs = {};
  let dropped = 0, mobilesDropped = 0;
  for (const e of elements) {
    ...
    (docs[k.slug].groups[g] ||= []).push(entryLine(e, subName ? subMeta[subName] : null));
  }
  const files = {};
  ...
  return { files, dropped, mobilesDropped };
}
...
if (require.main === module) (async () => {
  ...
  const { files, dropped, mobilesDropped } = build(data);
  ...
  console.log('unnamed places skipped: ' + dropped + ' · entries whose mobile number was left out: ' + mobilesDropped);
